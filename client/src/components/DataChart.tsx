...
export default function DataChart({
  ...
  emptyMessage = "No data available yet. Connect to a serial device to start streaming measurements.",
}: DataChartProps) {
  const shouldLimit = typeof maxDataPoints === "number" && maxDataPoints > 0;
  const limit = shouldLimit ? Math.min(maxDataPoints as number, data.length) : data.length;
  const displayData = limit === data.length ? data : data.slice(-limit);
